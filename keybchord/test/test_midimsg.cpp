...
TEST(MidiMsg, ChannelStatus) {
    EXPECT_EQ(midi::channelStatus(midi::STATUS_NOTE_ON, 0), 0x90);
    EXPECT_EQ(midi::channelStatus(midi::STATUS_NOTE_ON, 1), 0x91);
    EXPECT_EQ(midi::channelStatus(midi::STATUS_NOTE_ON, 15), 0x9F);
    EXPECT_EQ(midi::channelStatus(midi::STATUS_CONTROL_CHANGE, 0), 0xB0);
    EXPECT_EQ(midi::channelStatus(midi::STATUS_PROGRAM_CHANGE, 0), 0xC0);
}

TEST(MidiMsg, MakeNoteOn) {
    auto msg = midi::makeNoteOn(1, 60, 100);
    EXPECT_EQ(msg.status, 0x91);
...
TEST(MidiMsg, MakeNoteOff) {
    ...
    EXPECT_EQ(msg.status, 0x91);  // NoteOn with vel=0
    EXPECT_EQ(msg.data1, 60);
